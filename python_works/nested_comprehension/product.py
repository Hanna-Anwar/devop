...
allen_soly = [p.get("title") for p in clothes if p.get("brand").lower()=="Allen Solly".lower()]

print(allen_soly)

#display all product titleavailable in size s

size_s = [p.get("title") for p in clothes if "S" in p.get("sizes")]

print(size_s)

#in the given dataset which brand contain most number of product
...
# all_brands = ['Adidas', 'Van Heusen', "Levi's", 'Wrangler', 'H&M', 'Peter England', 'FabIndia', 'Nike', 'US Polo Assn.', 'Zara', 'Only', 'Forever 21', 'Reebok', 'Allen Solly', 'H&M', 'Woodland', 'Zara', 'Biba', 'Puma', 'North Face']

brand_count = {b:all_brands.count(b) for b in all_brands }

print(brand_count)

#brand_count =  {'Adidas': 1, 'Van Heusen': 1, "Levi's": 1, 'Wrangler': 1, 'H&M': 2, 'Peter England': 1, 'FabIndia': 1, 'Nike': 1, 'US Polo Assn.': 1, 'Zara': 2, 'Only': 1, 'Forever 21': 1, 'Reebok': 1, 'Allen Solly': 1, 'Woodland': 1, 'Biba': 1, 'Puma': 1, 'North Face': 1}


srted_bc = sorted(brand_count,key = brand_count.get,reverse=True)
